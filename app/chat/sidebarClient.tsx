...
export default function SidebarClient({ initialChats }: { initialChats: ChatItem[] }) {
	...
	async function saveRename() {
		...
		if (!id || !title) { setEditingId(null); return }
					...
					<div key={c.id} className={`group flex items-center justify-between px-3 py-2 border-b border-gray-800 hover:bg-gray-900 ${pathname === `/chat/${c.id}` ? 'bg-gray-900' : ''}`}>
							...
							{editingId === c.id ? (
								<div className="flex items-center gap-2 min-w-0">
									<input
										...
										className="w-36 rounded border border-white/10 bg-transparent px-2 py-1 text-sm"
									/>
									<button onClick={saveRename} className="rounded px-2 py-1 text-xs bg-white/10 hover:bg-white/20">Save</button>
									<button onClick={cancelRename} className="rounded px-2 py-1 text-xs hover:bg-white/10">Cancel</button>
								</div>
							) : (
								<>
						...
						<div className="opacity-0 group-hover:opacity-100">
							<details className="relative">
								<summary className="list-none rounded px-2 py-1 text-xs text-white/70 hover:bg-white/10 cursor-pointer" aria-label="Chat menu">⋯</summary>
								<div className="absolute right-0 z-10 mt-1 min-w-28 rounded-md border border-white/10 bg-gray-900 p-1 text-xs shadow-lg">
									<button onClick={() => beginRename(c.id, c.title)} className="block w-full rounded px-2 py-1 text-left hover:bg-white/10">Rename</button>
