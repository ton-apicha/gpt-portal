...
export default async function AdminLogsPage() {
    ...
    const u = new URL(`${base}/api/admin/logs`)
    const searchParams = hdrs.get('x-search-params') // not standard; fallback to none
    // Pull filters from query forwarded (Next doesn't expose directly; using request URL on server less trivial)
    // Instead, render empty inputs and client will fetch with filters
    ...
    return (
        <div className="p-6 text-white space-y-4">
            ...
            <LogsClient initialItems={items} base={base} />
        </div>
    )
}

function LogsClient({ initialItems, base }: { initialItems: any[]; base: string }){
    return (
        <div className="space-y-3">
            <Filters base={base} />
            <Table items={initialItems} />
        </div>
    )
}
